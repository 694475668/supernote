...
public final class CookieUtils {
        ...
        try {
            for (int i = 0; i < cookieList.length; i++) {
                if (cookieList[i].getName().equals(cookieName)) {
                	log.info("-----------------------domain="+ cookieList[i].getDomain());
                    if (isDecoder) {
    ...
    public static String createRedisKey(HttpServletRequest request) {
    	String rediskey = CookieUtils.getCookieValue(request,Constant.REDISKEY);
    	log.info("获取到的redisKey：{}", rediskey);
		if(StringUtils.isEmpty(rediskey)){
        	rediskey = UUID.randomUUID().toString().replaceAll("-", "");
        	log.info("新生成了redisKey：{}", rediskey);
        }
		return rediskey;
    }
}
